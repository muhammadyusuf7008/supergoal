...
Repeat until `SUPERPLAN_RUN_COMPLETE` is printed:

1. Read `.superplan/STATE.md`. Find `Current phase: N`.
2. Read `.superplan/phases/phase-N.md`. This is your full work spec.
3. Print `SUPERPLAN_PHASE_START` with the spec's metadata (phase number, name, task, mandatory commands, acceptance count, evidence types, dependencies).
...
5. Print `SUPERPLAN_PHASE_VERIFY`: each acceptance criterion `pass|fail` with evidence; engineering checks (build/typecheck/lint/tests); files changed count; notable diff one-liners.
...
7. Print `SUPERPLAN_PHASE_DONE`. Update `STATE.md`: mark phase N completed; set `Current phase: N+1`; bump `Last update` timestamp; append a one-line event.
...
10. If N == total: print `SUPERPLAN_RUN_COMPLETE` with a 5-line summary. The `/goal` condition is now satisfied.
...
2. Append the probe to `.superplan/STATE.md` failure log.
...
2. Write a focused **fix spec** at `.superplan/phases/phase-N.fix.md`. The fix spec:
...
- Pause at the next phase boundary (after `SUPERPLAN_PHASE_DONE`, before reading the next spec).
- Address the message.
...
- Save anything non-obvious a future Superplan run on a similar task would benefit from.
